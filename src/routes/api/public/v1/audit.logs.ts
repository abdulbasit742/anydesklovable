...
export const Route = createFileRoute("/api/public/v1/audit/logs")({
  server: {
    handlers: {
      GET: withPublicApi("read:audit", async (ctx, request) => {
        const url = new URL(request.url);
        const { limit, cursor } = parsePagination(url);
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        let q = supabaseAdmin.from("audit_logs").select("*").eq("team_id", ctx.teamId)
