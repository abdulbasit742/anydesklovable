...
export const Route = createFileRoute("/api/public/v1/automation/pipelines")({
  server: {
    handlers: {
      GET: withPublicApi("read:automation", async (ctx, request) => {
        const url = new URL(request.url);
        const { limit, cursor } = parsePagination(url);
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
        let q = supabaseAdmin.from("automation_pipelines").select("*").eq("team_id", ctx.teamId)
