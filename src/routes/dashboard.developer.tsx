              ...
              {keys.data.map((k) => {
                ...
                return (
                  <tr key={k.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-2"><StatusBadge variant={revoked ? "rejected" : expired ? "neutral" : "connected"}>{revoked ? "revoked" : expired ? "expired" : "active"}</StatusBadge></td>
...
function ScopeSelector({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  return <div className="grid gap-2 sm:grid-cols-2">{API_SCOPES.map((s) => <label key={s.value} className="flex items-center gap-2 rounded-md border border-border p-2 text-xs"><Checkbox checked={value.includes(s.value)} onCheckedChange={() => toggle(s.value)} />{s.label}</label>)}</div>;
}

// ---------- API Logs ----------
function ApiLogsTab() { return <SimplePanel title="API request logs" hook={() => { const q = useApiRequestLogs(); return { data: q.data, isLoading: q.isLoading, error: q.error }; }} columns={["method","path","status_code","latency_ms","created_at","error_code"]} />; }
function RateLimitsTab() { return <SimplePanel title="Rate limit events" hook={() => { const q = useApiRateLimitEvents(); return { data: q.data, isLoading: q.isLoading, error: q.error }; }} columns={["scope","limit_key","allowed","remaining","reset_at","created_at"]} />; }

function SimplePanel({ title, hook, columns }: { title: string; hook: () => { data?: any[]; isLoading: boolean; error: Error | null }; columns: string[] }) {
  const q = hook();
  const rows = q.data ?? [];
  return <div className="rounded-lg border border-border bg-card"><div className="border-b border-border px-4 py-3 text-sm font-semibold">{title}</div><PanelState loading={q.isLoading} error={q.error} empty={rows.length === 0} emptyText="No records yet."><div className="overflow-x-auto"><table className="w-full text-xs"><thead className="bg-muted/40 uppercase text-muted-foreground"><tr>{columns.map((c) => <th key={c} className="px-3 py-2 text-left font-medium">{c}</th>)}</tr></thead><tbody>{rows.map((r, i) => <tr key={r.id ?? i} className="border-t border-border">{columns.map((c) => <td key={c} className="px-3 py-2">{String(r[c] ?? "—")}</td>)}</tr>)}</tbody></table></div></PanelState></div>;
}
